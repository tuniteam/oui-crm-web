...
export const scenarios = [
...

  {
    id: '01-03.10',
    us: 'US-01-03',
    title: 'Annuler restaure tous les types de champ',
    needsProject: true,
    gherkin: [
      "Given j'ouvre la fiche d'un organisme",
    ...
    async run({ page, expect, projectId }) {
      ...
      // Cadre sur le panneau : la liste, derriere, a ses propres champs.
      const cancel = page.getByTestId('organization-cancel');
      const notes = sheet.locator('textarea').first();
      const prio = sheet
        .locator('button[role="combobox"]')
        .filter({ hasText: /Haute|Normale|Basse/ })
        .first();
      const box = sheet.locator('button[role="checkbox"]').first();

      const notesBefore = await notes.inputValue();
      const prioBefore = (await prio.innerText()).trim();
      const boxBefore = await box.getAttribute('data-state');

      await notes.fill('MODIFICATION DE TEST');

      // Le selecteur est le piege connu de cet ecran : un Radix passe de non
      // controle a controle peut ne pas suivre un `reset`.
      await prio.click();
      await page.waitForTimeout(400);
      const options = page.locator('[role="option"]');
      const count = await options.count();
      for (let i = 0; i < count; i += 1) {
        if ((await options.nth(i).innerText()).trim() !== prioBefore) {
      ...
      const notesAfter = await notes.inputValue();
      const prioAfter = (await prio.innerText()).trim();
      const boxAfter = await box.getAttribute('data-state');

      expect(notesAfter === notesBefore, `zone de texte non restauree : ${notesAfter}`);
      expect(prioAfter === prioBefore, `selecteur non restaure : ${prioAfter}`);
      expect(boxAfter === boxBefore, `case a cocher non restauree : ${boxAfter}`);
      expect(await sheet.isVisible(), 'le panneau a ete ferme au lieu de defaire la saisie');
    },
  },
