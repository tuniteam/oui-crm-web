...
/**
 * Surfaces pleines de la charte Oui Crm : degrade azur -> bleu ciel du logo,
 * filet clair en haut, ombre teintee. Les valeurs vivent dans
 * `theme.oui-crm.css` (tokens `--btn-*`), on ne fait que les cabler.
 *
 * Applique en compoundVariants et non dans le variant : les rendus
 * `appearance="ghost"` et `mode="link"` ne portent alors aucun fond a annuler.
 */
const PRIMARY_SURFACE = [
  'bg-[linear-gradient(to_bottom_right,var(--btn-primary-from),var(--btn-primary-to))]',
  'shadow-[var(--btn-primary-shadow)]',
  'hover:bg-[linear-gradient(to_bottom_right,var(--btn-primary-from-hover),var(--btn-primary-to-hover))]',
  'hover:shadow-[var(--btn-primary-shadow-hover)]',
  'data-[state=open]:bg-[linear-gradient(to_bottom_right,var(--btn-primary-from-hover),var(--btn-primary-to-hover))]',
...
const buttonVariants = cva(
  'cursor-pointer group whitespace-nowrap focus-visible:outline-hidden inline-flex items-center justify-center has-data-[arrow=true]:justify-between whitespace-nowrap text-sm font-medium ring-offset-background transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-60 [&_svg]:shrink-0',
  {
    variants: {
      ...
      size: {
        lg: 'h-10 rounded-[var(--btn-radius)] px-4 text-sm gap-1.5 [&_svg:not([class*=size-])]:size-4',
        md: 'h-8.5 rounded-[var(--btn-radius)] px-3 gap-1.5 text-[0.8125rem] leading-(--text-sm--line-height) [&_svg:not([class*=size-])]:size-4',
        sm: 'h-7 rounded-[calc(var(--btn-radius)-3px)] px-2.5 gap-1.25 text-xs [&_svg:not([class*=size-])]:size-3.5',
        icon: 'size-8.5 rounded-[var(--btn-radius)] [&_svg:not([class*=size-])]:size-4 shrink-0',
      },
      autoHeight: {
        true: '',
        false: '',
