...
interface AccordionMenuProps {
  ...
  onRootValueChange?: (values: string[]) => void;
}
...
function AccordionMenu({
  ...
  const [nestedStates, setNestedStates] =
    React.useState<Record<string, string | string[]>>(() => {
      if (props.type === 'multiple' && defaultRootValue) {
        return { ...initialNestedStates, root: defaultRootValue };
      }
      return initialNestedStates;
    });

  // L'init ci-dessus ne s'applique qu'au montage. Lors d'une navigation, on
  // ré-ouvre la chaîne de la sélection active — fusion additive : on n'ouvre
  // que, on ne referme jamais les sections déjà ouvertes par l'utilisateur.
  React.useEffect(() => {
    setNestedStates((prev) => {
      const next = { ...prev };
      for (const [key, value] of Object.entries(initialNestedStates)) {
        if (key === 'root' && props.type === 'multiple') {
          const prevArr = Array.isArray(prev.root)
            ? prev.root
            : prev.root
              ? [prev.root as string]
              : [];
          const add = Array.isArray(value) ? value : [value as string];
          next.root = Array.from(new Set([...prevArr, ...add]));
        } else {
          next[key] = value;
        }
      }
  ...
  }, [initialNestedStates, props.type]);
  const multipleValue = (
      ...
      {props.type === 'single' ? (
        ...
        <AccordionPrimitive.Root
          ...
          onValueChange={(value: string | string[]) => {
            setNestedStates((prev) => ({ ...prev, root: value }));
            onRootValueChange?.(value as string[]);
          }}
          {...props}
          role="menu"
        >
